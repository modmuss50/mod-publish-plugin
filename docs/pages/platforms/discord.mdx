  ...
  <Tab>
    ...
    publishMods {
        ...
        discord {
            ...
            // If you want to use an embed for the content instead (you can use both at the same time) use embedContent
            //embedContent = changelog.map { "# A new version of my cool mod has been released! \n" + it}
  ...
  <Tab>
   ...
   publishMods {
       ...
       discord {
           ...
           // Pass in the secret webhook URL to post the messages when running a dry run. Optional
           dryRunWebhookUrl.set(providers.environmentVariable("DISCORD_WEBHOOK_DRY_RUN"))

           // Set the username used to send the webhook, defaults to "Mod Publish Plugin"
           username.set("My Cool Mod")

           // Set the avatar image url for the webhook, defaults to none.
           avatarUrl.set("https://placekitten.com/500/500")

           // Set the content message, in this example a header is added before the changelog. Defaults to just the changelog
           content.set(changelog.map { "# A new version of my cool mod has been released! \n" + it})

           // If you want to use an embed for the content instead (you can use both at the same time) use embedContent
           //embedContent.set(changelog.map { "# A new version of my cool mod has been released! \n" + it})
        ...
        publishMods {
          ...
          discord {
            ...
            style {
              // ...
              look.set("MODERN")
              thumbnailUrl.set("https://example.com")
              color.set("#f6f0fc")
            }
          }
        }
